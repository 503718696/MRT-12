...
class MRT12_Layer(nn.Module):
    def __init__(self, d_model, layer_idx):
        ...
        self.binder = CausalConv1d(d_model, k=3)  # 因果卷积
        self.concept_act = nn.Sequential(
            nn.Linear(d_model, d_model * 2),
            nn.SiLU(),
            nn.Linear(d_model * 2, d_model)
        )
        self.norm = RMSNorm(d_model)  # 流形归一化
...
**优势**：
- ✅ **几何架构**：更复杂的推理能力（弯曲空间表示）
- ✅ **因果性保证**：严格的时间依赖约束（左侧 padding）
- ✅ **内存高效**：针对消费级 GPU 优化（RTX 3090 Ti）
  - 训练显存：~7.8GB（BS=8, seq_len=512）
  - KV Cache：~0.03GB（极低）
- ✅ **生产就绪**：完整训练流水线（梯度检查点、自动配置）
...
| 场景 | Transformer | Mamba | HySparse | MRT |
|------|------------|-------|----------|-----|
| **短文本分类** | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐ |
| **机器翻译** | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐ |
| **长文档理解** | ⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ |
...
- ✅ **首选**: MRT（专门优化）、Mamba（显存友好）
- ✅ **可选**: HySparse（中长文本）
- ❌ **不推荐**: Transformer（长上下文 OOM）
...
**入门资料**：
- 📖 技术文档：`docs/MRT12_TECHNICAL_DOCUMENTATION.md`
- 💻 代码：`core/model_mrt12.py`
- 📘 背景知识：黎曼几何、范畴论、流形学习
