...
### 数据文件 (data/ 目录)
- `zhwiki_dataset.jsonl` (2.4GB): 中文维基百科训练语料（1200 万句） ⬇️ **从夸克网盘下载**
- `mrt_vocab.json` (201KB): 模型词汇表（7,429 tokens）

### 预训练模型 (models/ 目录)
- `mrt12_step_070000_20260301_213409.pth` (~2.3GB): ⬇️ **从夸克网盘下载**
  - 训练步数：70,000 步
  - 最终 Loss: 3.346
...
由于 GitHub 对文件大小有限制（单文件≤100MB），以下大型文件请从夸克网盘下载：

**🔗 夸克网盘链接**: https://pan.quark.cn/s/c9101da1efe2

**需要下载的文件**：
1. `mrt12_step_070000_20260301_213409.pth` (~2.3GB) - 预训练模型检查点
2. `zhwiki_dataset.jsonl` (~2.4GB) - 中文维基百科训练语料

**下载后放置位置**：
```
MRT12_FULL_RELEASE/
├── data/
│   └── zhwiki_dataset.jsonl  # 放在此处
└── models/
    └── mrt12_step_070000_20260301_213409.pth  # 放在此处
```

**💡 提示**：打开「夸克 APP」，无需下载即可在线查看，支持 5 倍速播放和电视投屏。
